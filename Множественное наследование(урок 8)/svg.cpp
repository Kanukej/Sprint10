...
void Snowman::Draw(svg::ObjectContainer& container) const {
    container.Add(svg::Circle().SetCenter({center_.x, center_.y + 5 * rad_}).SetRadius(2.0 * rad_));
    container.Add(svg::Circle().SetCenter({center_.x, center_.y + 2 * rad_}).SetRadius(1.5 * rad_));
    container.Add(svg::Circle().SetCenter(center_).SetRadius(rad_));
}
